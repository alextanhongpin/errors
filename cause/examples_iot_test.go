...
func (idc *IoTDeviceConfig) Validate() error {
	return cause.Map{
		"device_id": cause.Required(idc.DeviceID).
			When(!isValidDeviceID(idc.DeviceID), "invalid device ID format").
			When(len(idc.DeviceID) > 50, "device ID too long"),

		"device_name": cause.Required(idc.DeviceName).
			When(len(idc.DeviceName) < 3, "device name too short").
			When(len(idc.DeviceName) > 100, "device name too long").
			When(containsSpecialChars(idc.DeviceName), "device name contains invalid characters"),

		"device_type": cause.Required(idc.DeviceType).
			When(!isValidDeviceType(idc.DeviceType), "invalid device type"),

		"model": cause.Required(idc.Model).
			When(len(idc.Model) < 2, "model name too short").
			When(len(idc.Model) > 50, "model name too long"),

		"manufacturer": cause.Required(idc.Manufacturer).
			When(len(idc.Manufacturer) < 2, "manufacturer name too short").
			When(len(idc.Manufacturer) > 50, "manufacturer name too long"),

		"firmware_version": cause.Required(idc.FirmwareVersion).
			When(!isValidVersion(idc.FirmwareVersion), "invalid firmware version format"),

		"network_config":  cause.Required(idc.NetworkConfig),
		"sensor_config":   cause.Optional(idc.SensorConfig),
		"security_config": cause.Required(idc.SecurityConfig),
		"power_config":    cause.Required(idc.PowerConfig),
		"location_info":   cause.Optional(idc.LocationInfo),
		"metadata":        cause.Required(idc.Metadata),

		"status": cause.Required(idc.Status).
			When(!isValidDeviceStatus(idc.Status), "invalid device status"),

		"last_seen": cause.Required(idc.LastSeen).
			When(idc.LastSeen.After(time.Now()), "last seen cannot be in the future").
			When(idc.LastSeen.Before(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)), "last seen date too old"),

		"registered_at": cause.Required(idc.RegisteredAt).
			When(idc.RegisteredAt.After(time.Now()), "registration date cannot be in the future").
			When(idc.RegisteredAt.Before(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)), "registration date too old"),
	}.Err()
}
...
func (nc *NetworkConfig) Validate() error {
	return cause.Map{
		"wifi_ssid": cause.Optional(nc.WiFiSSID).
			When(len(nc.WiFiSSID) > 32, "WiFi SSID too long").
			When(containsInvalidSSIDChars(nc.WiFiSSID), "WiFi SSID contains invalid characters"),

		"wifi_password": cause.Optional(nc.WiFiPassword).
			When(len(nc.WiFiPassword) < 8 && len(nc.WiFiPassword) > 0, "WiFi password too short").
			When(len(nc.WiFiPassword) > 63, "WiFi password too long"),

		"ip_address": cause.Optional(nc.IPAddress).
			When(!isValidIPAddress(nc.IPAddress), "invalid IP address format"),

		"subnet_mask": cause.Optional(nc.SubnetMask).
			When(!isValidIPAddress(nc.SubnetMask), "invalid subnet mask format"),

		"gateway": cause.Optional(nc.Gateway).
			When(!isValidIPAddress(nc.Gateway), "invalid gateway IP address"),

		"dns_servers": cause.Optional(nc.DNSServers).
			When(len(nc.DNSServers) > 5, "too many DNS servers").
			When(!areValidDNSServers(nc.DNSServers), "invalid DNS server addresses"),

		"mac_address": cause.Required(nc.MACAddress).
			When(!isValidMACAddress(nc.MACAddress), "invalid MAC address format"),

		"network_mode": cause.Required(nc.NetworkMode).
			When(!isValidNetworkMode(nc.NetworkMode), "invalid network mode"),

		"signal_strength": cause.Optional(nc.SignalStrength).
			When(nc.SignalStrength < -100 || nc.SignalStrength > 0, "signal strength outside valid range"),

		"bandwidth_mbps": cause.Optional(nc.Bandwidth).
			When(nc.Bandwidth < 0, "bandwidth cannot be negative").
			When(nc.Bandwidth > 10000, "bandwidth exceeds reasonable limit"),
	}.Err()
}
...
func (sc *SensorConfig) Validate() error {
	return cause.Map{
		"sensor_id": cause.Required(sc.SensorID).
			When(!isValidSensorID(sc.SensorID), "invalid sensor ID format").
			When(len(sc.SensorID) > 30, "sensor ID too long"),

		"sensor_type": cause.Required(sc.SensorType).
			When(!isValidSensorType(sc.SensorType), "invalid sensor type"),

		"sensor_name": cause.Required(sc.SensorName).
			When(len(sc.SensorName) < 2, "sensor name too short").
			When(len(sc.SensorName) > 50, "sensor name too long"),

		"unit": cause.Required(sc.Unit).
			When(!isValidUnit(sc.Unit), "invalid measurement unit"),

		"sample_rate_seconds": cause.Required(sc.SampleRate).
			When(sc.SampleRate < 1, "sample rate must be at least 1 second").
			When(sc.SampleRate > 86400, "sample rate cannot exceed 24 hours"),

		"threshold":   cause.Required(sc.Threshold),
		"calibration": cause.Required(sc.Calibration),

		"location": cause.Optional(sc.Location).
			When(len(sc.Location) > 100, "sensor location description too long"),
	}.Err()
}
...
func (st *SensorThreshold) Validate() error {
	return cause.Map{
		"min_value": cause.Required(st.MinValue).
			When(st.MinValue >= st.MaxValue, "minimum value must be less than maximum value"),

		"max_value": cause.Required(st.MaxValue),

		"critical_min": cause.Required(st.CriticalMin).
			When(st.CriticalMin > st.MinValue, "critical minimum must be less than or equal to minimum value"),

		"critical_max": cause.Required(st.CriticalMax).
			When(st.CriticalMax < st.MaxValue, "critical maximum must be greater than or equal to maximum value"),
	}.Err()
}
...
func (sc *SensorCalibration) Validate() error {
	return cause.Map{
		"scale": cause.Required(sc.Scale).
			When(sc.Scale <= 0, "calibration scale must be positive"),
		...
		"last_calibrated": cause.Required(sc.LastCalibrated).
			...
			When(sc.LastCalibrated.Before(time.Date(1990, 1, 1, 0, 0, 0, 0, time.UTC)), "calibration date too old"),

		"calibrated_by": cause.Required(sc.CalibratedBy).
			When(len(sc.CalibratedBy) < 2, "calibrated by field too short").
			When(len(sc.CalibratedBy) > 50, "calibrated by field too long"),

		"calibration_cert": cause.Optional(sc.CalibrationCert).
			When(len(sc.CalibrationCert) > 100, "calibration certificate ID too long"),
	}.Err()
}
...
func (sec *SecurityConfig) Validate() error {
	return cause.Map{
		"encryption_type": cause.RequiredMessage(sec.EncryptionType, "encryption type required when encryption enabled").
			When(sec.EncryptionEnabled && !isValidEncryptionType(sec.EncryptionType), "invalid encryption type"),
		...
		"certificate_id": cause.Optional(sec.CertificateID).
			...
			When(len(sec.CertificateID) > 50, "certificate ID too long"),

		"auth_method": cause.Required(sec.AuthMethod).
			When(!isValidAuthMethod(sec.AuthMethod), "invalid authentication method"),

		"api_keys": cause.Optional(sec.APIKeys).
			When(len(sec.APIKeys) > 10, "too many API keys"),
		...
		"last_security_scan": cause.Optional(sec.LastSecurityScan).
			...
			When(!sec.LastSecurityScan.IsZero() && sec.LastSecurityScan.Before(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)), "security scan date too old"),
	}.Err()
}
...
func (ak *APIKey) Validate() error {
	return cause.Map{
		"key_id": cause.Required(ak.KeyID).
			...
			When(len(ak.KeyID) > 50, "API key ID too long"),

		"key_name": cause.Required(ak.KeyName).
			When(len(ak.KeyName) < 3, "API key name too short").
			When(len(ak.KeyName) > 50, "API key name too long"),
		...
		"permissions": cause.Required(ak.Permissions).
			...
			When(!areValidPermissions(ak.Permissions), "invalid permissions"),
		...
		"expires_at": cause.Required(ak.ExpiresAt).
			...
			When(ak.ExpiresAt.After(time.Now().AddDate(5, 0, 0)), "expiry date too far in future"),

		"created_at": cause.Required(ak.CreatedAt).
			When(ak.CreatedAt.After(time.Now()), "creation date cannot be in the future").
			When(ak.CreatedAt.Before(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC)), "creation date too old"),
	}.Err()
}
...
func (pc *PowerConfig) Validate() error {
	return cause.Map{
		"power_source": cause.Required(pc.PowerSource).
			When(!isValidPowerSource(pc.PowerSource), "invalid power source"),
		...
		"battery_level": cause.Optional(pc.BatteryLevel).
			...
			When(pc.PowerSource == "Battery" && pc.BatteryLevel > 100, "battery level cannot exceed 100%"),

		"voltage_rating": cause.Required(pc.VoltageRating).
			When(pc.VoltageRating <= 0, "voltage rating must be positive").
			When(pc.VoltageRating > 1000, "voltage rating exceeds safe limits"),
		...
		"power_consumption_watts": cause.Required(pc.PowerConsumption).
			...
			When(pc.PowerConsumption > 10000, "power consumption exceeds reasonable limits"),
		...
		"sleep_schedule": cause.Optional(pc.SleepSchedule).
			...
			When(!isValidCronSchedule(pc.SleepSchedule), "invalid sleep schedule format"),
		...
		"power_threshold_percent": cause.Optional(pc.PowerThreshold).
			...
			When(pc.PowerThreshold < 0 || pc.PowerThreshold > 100, "power threshold must be between 0 and 100"),
	}.Err()
}
...
func (li *LocationInfo) Validate() error {
	return cause.Map{
		"building": cause.Optional(li.Building).
			When(len(li.Building) > 100, "building name too long"),

		"room": cause.Optional(li.Room).
			When(len(li.Room) > 50, "room identifier too long"),

		"floor": cause.Optional(li.Floor).
			When(li.Floor < -10 || li.Floor > 200, "floor number outside reasonable range"),

		"zone": cause.Optional(li.Zone).
			When(len(li.Zone) > 50, "zone identifier too long"),

		"latitude": cause.Optional(li.Latitude).
			When(li.Latitude < -90 || li.Latitude > 90, "latitude outside valid range"),

		"longitude": cause.Optional(li.Longitude).
			When(li.Longitude < -180 || li.Longitude > 180, "longitude outside valid range"),

		"altitude": cause.Optional(li.Altitude).
			When(li.Altitude < -1000 || li.Altitude > 10000, "altitude outside reasonable range"),

		"address": cause.Optional(li.Address).
			When(len(li.Address) > 200, "address too long"),

		"description": cause.Optional(li.Description).
			When(len(li.Description) > 500, "location description too long"),
	}.Err()
}
...
func (dm *DeviceMetadata) Validate() error {
	return cause.Map{
		"tags": cause.Optional(dm.Tags).
			...
			When(!areValidTags(dm.Tags), "invalid tag format"),

		"environment": cause.Required(dm.Environment).
			When(!isValidEnvironment(dm.Environment), "invalid environment"),

		"deployment_type": cause.Required(dm.DeploymentType).
			When(!isValidDeploymentType(dm.DeploymentType), "invalid deployment type"),

		"maintenance_window": cause.Optional(dm.MaintenanceWindow).
			When(!isValidCronSchedule(dm.MaintenanceWindow), "invalid maintenance window format"),

		"support_contact": cause.Optional(dm.SupportContact).
			When(!isValidEmail(dm.SupportContact), "invalid support contact email"),
		...
		"purchase_date": cause.Optional(dm.PurchaseDate).
			...
			When(!dm.PurchaseDate.IsZero() && dm.PurchaseDate.Before(time.Date(1990, 1, 1, 0, 0, 0, 0, time.UTC)), "purchase date too old"),
		...
		"warranty_expiry": cause.Optional(dm.WarrantyExpiry).
			...
			When(!dm.WarrantyExpiry.IsZero() && dm.WarrantyExpiry.After(time.Now().AddDate(10, 0, 0)), "warranty period too long"),

		"custom_fields": cause.Optional(dm.CustomFields).
			When(len(dm.CustomFields) > 50, "too many custom fields"),
	}.Err()
}
...
func isValidDeviceType(deviceType string) bool {
	validTypes := []string{
		"Sensor", "Camera", "Gateway", "Controller", "Display",
		"Actuator", "Switch", "Relay", "Monitor", "Hub",
	}
